...
export class TypeLoadService {
  typeLoad: TypeLoad;
  info: any;
  public edit: boolean = false;
  public type: string = "";
  private urlBase = environment.publicUrl;
  private namespace = "budget/type/load";
  private url = "private/budget/type/load";
  ...
  add(data: TypeLoad): Observable<any> {
    if (!navigator.onLine) {
  ...
  create(data: TypeLoad): Observable<any> {
  ...
  update(data: TypeLoad): Observable<any> {
  ...
  getList(categoryUudi?,budget?, type?, trustee?): Observable<TypeLoad[]>  {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getSingle(uuid: string): Observable<TypeLoad> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getListTask(uuid: string): Observable<TypeLoad> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getExport(agencyKey: string, userKey: string, data: any): void {
    ...
    let url = 'export/typeLoad/' + agencyKey + '/' + userKey;
    if(data && data !== undefined) {
      for (const k in data) {
        if (data.hasOwnProperty(k)) {
          if(k !== 'uuid') {
