...
export class OwnerCoService {
  ...
  private namespace = "agency/ownerco";
  private url = "private/agency/ownerco";

  constructor(private api: ApiService) {
  }

  setOwnerCo(ownerCo: OwnerCo) {
    this.ownerCo = ownerCo;
  }
  getOwnerCo(): OwnerCo {
    return this.ownerCo;
  }
  add(data: OwnerCo): Observable<any> {
    if (!navigator.onLine) {
  ...
  create(data: OwnerCo): Observable<any> {
    return this.api._post(`${this.url}/new`, data).pipe(
      map((response: any) => response),
      catchError((error: any) => throwError(error))
    );
  }
  update(data: OwnerCo): Observable<any> {
  ...
  getPrinter(type: string, agencyKey: string, userKey: string, uuid: string, data: any): void {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return;
    }
