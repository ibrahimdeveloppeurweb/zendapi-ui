...
export class LotService {
  ...
  private namespace = "agency/lot";
  private url = "private/agency/lot";
  ...
  getList(subdivisonUuid ?: string, isletUuid ?: string, all = 'LIST'): Observable<Lot[]> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getCoords(subdivisonUuid ?: string, isletUuid ?: string): Observable<Lot[]> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getPrinter(type: string, agencyKey: string, userKey: string, data: any): void {
    ...
    if(type === 'LISTE') {
      if(data && data !== undefined) {
        ...
        url += '/LOT/null/null/null/null/null/null/null/null/null/DESC/null/null/null/null/10/null'
      }
    }
    if(type === 'SHOW') {
  ...
  getExport(agencyKey: string, userKey: string, data: any): void {
    ...
    let url = 'export/lot/' + agencyKey + '/' + userKey;
    if(data && data !== undefined) {
      for (const k in data) {
        if (data.hasOwnProperty(k)) {
          if(k !== 'uuid') {
  ...
  getGenerer(){
    ...
    var url = this.urlBase + '/import/agency/model/lot'
    window.open(`${url}`);
  }
  ...
  import(data){
    ...
    return this.api._post(`${url}/lot`, data).pipe(
      map((response: any) => response),
      catchError((error: any) => throwError(error))
    );
  }

  getSingle(uuid: string): Observable<Lot> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
