...
export class WorksiteService {
  ...
  private namespace = "agency/worksite";
  private url = "private/agency/worksite";

  constructor(private api: ApiService) { }

  setWorksite(worksite: Worksite) {
    this.worksite = worksite
  }

  getWorksite(): Worksite {
    return this.worksite
  }

  add(data: Worksite): Observable<any> {
    if (!navigator.onLine) {
  ...
  create(data: Worksite): Observable<any> {
  ...
  update(data: Worksite): Observable<any> {
  ...
  getSingle(uuid: string): Observable<Worksite> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getListTask(uuid: string): Observable<Worksite> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getExport(agencyKey: string, userKey: string, data: any): void {
    ...
    let url = 'export/worksite/' + agencyKey + '/' + userKey;
    if(data && data !== undefined) {
      for (const k in data) {
        if (data.hasOwnProperty(k)) {
          if(k !== 'uuid') {
