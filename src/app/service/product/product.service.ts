...
export class ProductService {
  ...
  private namespace = "agency/product";
  private url = "private/agency/product";
  ...
  getSingle(uuid: string): Observable<Product> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getPrinter(type: string, agencyKey: string, userKey: string, data: any): void {
    ...
    if(type === 'LISTE') {
      if(data && data !== undefined) {
        ...
        url += '/PRODUIT/null/null/null/null/null/null/null/DESC/null/null/null/null/10/null'
      }
    }
    if(type === 'SHOW') {
  ...
  getExport(agencyKey: string, userKey: string, data: any): void {
    ...
    let url = 'export/product/' + agencyKey + '/' + userKey;
    if(data && data !== undefined) {
      for (const k in data) {
        if (data.hasOwnProperty(k)) {
          if(k !== 'uuid') {
  ...
  getGenerer(){
    ...
    var url = this.urlBase + '/import/agency/model/product'
    window.open(`${url}`);
  }
  ...
  import(data){
    ...
    return this.api._post(`${url}/product`, data).pipe(
      map((response: any) => response),
      catchError((error: any) => throwError(error))
    );
  }
  getDelete(uuid: string): Observable<Product> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
