...
export class DepositService {
  ...
  private namespace = "agency/deposit";
  private url = "private/agency/deposit";
  type: any;
  ...
  add(data: Deposit): Observable<any> {
    if (!navigator.onLine) {
  ...
  create(data: Deposit): Observable<any> {
    return this.api._post(`${this.url}/new`, data).pipe(
      map((response: any) => response),
      catchError((error: any) => throwError(error))
    );
  }
  update(data: Deposit): Observable<any> {
  ...
  getList(ownerUuid: string = null): Observable<Deposit[]> {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return Observable.create(obs => {
        obs.next();
  ...
  getReleve(type: string, agencyKey: string, userKey: string, entity: string=null, dateD: string=null, dateF: string=null): void {
    if (!navigator.onLine) {
      NoInternetHelper.internet()
      return;
    }
    let url = 'printer/agency/owner/releve/' + agencyKey + '/' + userKey;
    url =  url +'/' + entity + '/' + dateD  + '/' + dateF ;
