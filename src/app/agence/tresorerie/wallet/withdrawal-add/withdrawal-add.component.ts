...
@Component({
  ...
  styleUrls: ['./withdrawal-add.component.scss']
})
export class WithdrawalAddComponent implements OnInit {
  ...
  constructor(
    ...
    private withdrawService: WithdrallService,
  ) {
    this.edit = this.withdrawService.edit;
    this.owner = this.ownerService.getOwner()
    this.treasury = this.treasuryService.getTreasury()
    this.title = (!this.edit) ? "Effectuer un retrait" : "Modifier le retrait";
  ...
  editForm(){
    if (this.edit) {
      const data = {...this.withdrawService.getWithdrawll() };
      console.log(data);
  ...
  onSubmit() {
    this.submit = true;
    if (this.form.valid) {
      this.withdrawService.add(this.form.getRawValue()).subscribe(res => {
        if (res?.status === 'success') {
          this.onClose();
          if (this.form.value.uuid) {
            this.emitter.emit({action: 'WITHDRAWLL_UPDATED', payload: res?.data});
          } else {
            this.emitter.emit({action: 'WITHDRAW_ADD', payload: res?.data});
          }
        }
      }, error => {});
    } else {
