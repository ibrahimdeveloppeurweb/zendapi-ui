...
@Component({
  selector: 'app-syndic-budget-list',
  templateUrl: './syndic-budget-list.component.html',
  styleUrls: ['./syndic-budget-list.component.scss'],
})
export class SyndicBudgetListComponent implements OnInit {
  ilot: boolean = false;
  lot: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.emitter.event.subscribe((data) => {
      if (data.action === 'BUDGET_ADD') {
        this.budgets.unshift(data.payload);
      }

      if (data.action === 'BUDGET_UPDATE') {
        const index = this.budgets.findIndex(x => x.uuid === data?.payload?.uuid);
        if (index !== -1) {
          this.budgets[index] = data?.payload;
        }
      }
  ...
  show(row, type) {
    ...
    if (type == 'SHOW') {
      this.router.navigate(['/admin/syndic/budget/show/' + row.uuid]);
    }else {
    ...
    Swal.fire({
      ...
      if (willDelete.dismiss) {
        ...
        this.budgetService.getDelete(item.uuid).subscribe(res => {
          if (res?.status === 'success') {
                  const index = this.budgets.findIndex(x => x.id === item.id);
                  if (index !== -1) { this.budgets.splice(index, 1); }
