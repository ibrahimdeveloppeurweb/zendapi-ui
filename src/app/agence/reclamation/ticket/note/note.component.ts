...
export class NoteComponent implements OnInit {
  ...
  public uploadService: UploaderService
    ...
    this.title = "Faire une note sur le ticket";
    this.newForm()
    this.optionTickets = this.ticketService.optionTickets
  ...
  onSubmit() {
    this.submit = true;
    if (this.form.valid) {
      this.ticketService.note(this.form.value).subscribe(
        res => {
          if (res?.status === 'success') {
            this.modal.dismiss();
            this.modal.close('ferme');
            this.emitter.emit({action: 'TICKET_NOTE', payload: res?.data});
