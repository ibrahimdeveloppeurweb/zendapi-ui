...
@Component({
  selector: 'app-grouped-contract-add',
  templateUrl: './grouped-contract-add.component.html',
  styleUrls: ['./grouped-contract-add.component.scss'],
  providers: [DatePipe]
})
export class GroupedContractAddComponent implements OnInit {
  title: string = "";
  caution = 0;
  form: FormGroup;
  submit = false;
  ...
  arrayOfSelectedLocatives: any[] = []

  constructor(
    public modal: NgbActiveModal,
    private formBuild: FormBuilder,
    private contractService: GroupedContractService,
    ...
    this.title = (!this.edit) ? 'Ajouter un contrat groupé' : 'Modifier le contrat groupé de ' + this.contract?.tenant?.nom;
    this.newForm();
  }
  ...
  setOwnerUuid(uuid) {
    if(uuid){
      ...
      this.f.block.setValue(null);
      this.f.rentals.setValue(null);
    }
  }
      ...
      this.house = this.houses.find(item => {
        if (item.uuid === value) {
          if (item?.resilie) {
            ...
            if(!this.edit) {
              this.rentalService.getList(this.f.owner.value, item.uuid, 'ALL').subscribe((res:any) => {
                console.log(res)
                if (res) {
                  res.forEach(val => {
  ...
  onSelectRentals(event: string[]) {
    console.log(event);

    let m = 0; // loyer
    let c = 0; // charges
    let t = 0; // total
    let p = 0; // pas de porte

    // Reset tableau si utilisé
    this.arrayOfSelectedLocatives = [];

    if (event && event.length > 0) {
      event.forEach(selectedUuid => {
        const item: any = this.rentals.find((r: any) => r.uuid === selectedUuid);
        if (item) {
          this.arrayOfSelectedLocatives.push(item);
          m += item?.montant || 0;
          c += item?.charge || 0;
          t += item?.total || 0;
          p += parseInt(item?.pasPorte) || 0;
        }
      });
    }

    // Mise à jour du formulaire et des totaux
    this.f.loyer.setValue(m);
    this.f.charge.setValue(c);
    this.f.loyerCharge.setValue(t);
    this.f.pasPorte.setValue(p);
  ...
  onChangeTotal() {
    ...
    this.option.controls.forEach(elem => {
      const qte = elem.value.qte || 0;
      const prix = elem.value.prix || 0;
      const remise = elem.value.remise >= 0 ? elem.value.remise : 0;
      const tvaRate = elem.value.tva >= 0 ? elem.value.tva : 0;

      const totalHt = (prix * qte) - remise;
    ...
    const pasDePorte = parseFloat(this.f.pasPorte.value) || 0;
    this.totalTTC = pasDePorte + totalOptionTTC;
  ...
  onRentGenere(data) {
    ...
    const nbMois = this.monthDiffGlobal(this.f.dateEntr.value)
    const moisEntr = new Date(this.f.dateEntr.value).getMonth() + 1;
    ...
    if (!this.edit && this.form?.value?.loyer) {
      for(let i = 0; i < this.libelleRow.length; i++){
        ...
        if (this.libelleRow[i].value === 2.5) {
          if(this.form?.value?.type === 'COMMERCIAL') {
